...
WHERE EXISTS (
    SELECT 1
    FROM pago p2
    WHERE p1.codigo_cliente = p2.codigo_cliente
    AND p2.fecha_pago >= '2008-01-01' AND p2.fecha_pago <= '2008-12-31'
);
...
SELECT p.nombre, p.cantidad_en_stock, p.precio_venta
FROM producto p
...
SELECT c.nombre_cliente, c.ciudad, e.codigo_empleado
FROM cliente c
JOIN empleado e ON c.codigo_empleado_rep_ventas = e.codigo_empleado
WHERE c.ciudad = 'Madrid' AND (e.codigo_empleado = 11 OR e.codigo_empleado = 30);
...
SELECT c.nombre_cliente, GROUP_CONCAT(DISTINCT p.gama) AS gamas_compradas
FROM cliente c
...
WHERE e.codigo_empleado IS NULL
   AND o.codigo_oficina NOT IN (
        SELECT o2.codigo_oficina
        FROM oficina o2
        JOIN empleado e2 ON o2.codigo_oficina = e2.codigo_oficina
        JOIN cliente c ON e2.codigo_empleado = c.codigo_empleado_rep_ventas
        JOIN pedido p ON c.codigo_cliente = p.codigo_cliente
...
FROM empleado e1
LEFT JOIN empleado e2 ON e1.codigo_jefe = e2.codigo_empleado
WHERE e1.codigo_empleado_rep_ventas IS NULL;
Consultas resumen
...
cada uno.
SELECT e.nombre, COUNT(c.codigo_cliente) AS clientes_atendidos
FROM empleado e
LEFT JOIN cliente c ON e.codigo_empleado = c.codigo_empleado_rep_ventas
...
SELECT p.codigo_pedido, COUNT(DISTINCT dp.codigo_producto) AS productos_diferentes
FROM pedido p
JOIN detalle_pedido dp ON p.codigo_pedido = dp.codigo_pedido
...
SELECT p.nombre
FROM producto p
WHERE p.codigo_producto = (SELECT dp.codigo_producto
    FROM detalle_pedido dp
